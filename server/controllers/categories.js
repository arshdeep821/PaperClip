...
const createCategory = async (req, res) => {
    try {
        ...
        if (existing) {
            return res.status(StatusCodes.CONFLICT).json({ error: 'Category already exists.' });
        }
...
const getAllCategories = async (req, res) => {
    try {
        ...
        res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ error: 'Server error. Could not fetch categories.' });
    }
};
