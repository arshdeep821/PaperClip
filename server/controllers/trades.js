...
const executeTrade = async (req, res) => {
	try {
		...
		if (!user1) {
			return res
				.status(StatusCodes.NOT_FOUND)
				.json({ error: "User1 not found." });
		}
		if (!user2) {
			return res
				.status(StatusCodes.NOT_FOUND)
				.json({ error: "User2 not found." });
		}
		if (items1.length !== items1Id.length) {
			return res
				.status(StatusCodes.NOT_FOUND)
				.json({ error: "Some items in items1 were not found." });
		}
		if (items2.length !== items2Id.length) {
			return res
				.status(StatusCodes.NOT_FOUND)
				.json({ error: "Some items in items2 were not found." });
		}

		const isUser1OwnerOfAllItems1 = items1Id.every((id) =>
			user1.inventory.includes(id.toString())
		);
		const isUser2OwnerOfAllItems2 = items2Id.every((id) =>
			user2.inventory.includes(id.toString())
		);

		if (!isUser1OwnerOfAllItems1) {
			return res
				.status(StatusCodes.BAD_REQUEST)
				.json({ error: "user1 is not the owner of all items1" });
		}
		if (!isUser2OwnerOfAllItems2) {
			return res
				.status(StatusCodes.BAD_REQUEST)
				.json({ error: "user2 is not the owner of all items2" });
		}

		// swaps items from user.inventory
		user1.inventory = user1.inventory.filter(
			(itemId) => !items1Id.includes(itemId.toString())
		);
		user2.inventory = user2.inventory.filter(
			(itemId) => !items2Id.includes(itemId.toString())
		);
