...
function Messages() {
    ...
    const fetchConversations = async () => {
        try {
            setLoading(true);
            const response = await fetch(`${BACKEND_URL}/messages/conversations/${currentUser.id}`);
            if (response.ok) {
    ...
    const loadMessages = async (otherUserId) => {
        try {
            const response = await fetch(`${BACKEND_URL}/messages/${currentUser.id}/${otherUserId}`);
            if (response.ok) {
                const data = await response.json();
                setMessages(data);
            }
    ...
    const updateConversationWithNewMessage = (msg) => {
        setConversations(prev =>
            prev.map(conv => {
                if (conv.otherUser._id === msg.from || conv.otherUser._id === msg.to) {
                    return {
                        ...conv,
                        lastMessage: msg,
                        unreadCount: selectedConversation?.otherUser._id === conv.otherUser._id
                            ? 0
                            : conv.unreadCount + 1
                    };
                }
    ...
    for (const conv of conversations) {
        if (conv.otherUser._id !== currentUser.id && !seen.has(conv.otherUser._id)) {
                    ...
                    {uniqueConversations.length === 0 ? (
                        ...
                        uniqueConversations.map((conversation) => (
                            <div
                                ...
                                className={`${styles.conversationItem} ${selectedConversation?.otherUser._id === conversation.otherUser._id
                                        ? styles.selected
                                        : ''
                                    }`}
                        ...
                        <>
                            ...
                            <div className={styles.messagesContainer}>
                                {messages.map((msg, idx) => (
                                    <div
                                        key={idx}
                                        className={`${styles.message} ${msg.from === currentUser.id ? styles.sent : styles.received
                                            }`}
                                    >
